...
| AI Explanation         | Gemini API (Google, Free Tier) via `@google/generative-ai`                          |
...
- Integration with Gemini API (free tier) to turn structured risk JSON (from Members 1 & 2) into a plain-English explanation + recommendation
...
- Dashboard webview (React) showing list of risks, affected files, severity, and project health overview
  **Antigravity prompt focus:** "Build a VS Code extension that watches for file saves/commits, calls a local analysis API, sends the structured risk output to the Gemini API for a natural-language explanation, and displays results both inline and in a React-based dashboard webview."
