...
graph TD
    subgraph Client [VS Code Extension Frontend]
        VSC[VS Code Workspace Workspace] -->|Watches Working Tree / Branch Diffs| GDT[Git Change Detection]
    ...
    subgraph Backend [Core Analysis Engine]
        API_Client -->|POST /analyze| REST[Express API / FastAPI]
...
- **Webview Dashboard**: A React-based web panel that lists all active repository-wide risks, categorized by branch status and severity level.

### 4.2 Core Engine Layer (Local Backend API)

- **AST Parser Module**: Resolves changed lines back to AST coordinates. Supports JavaScript/TypeScript using `@babel/parser` and standard compiler APIs.
